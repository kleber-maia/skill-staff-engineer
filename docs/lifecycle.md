...
| `verify --mode full` | unchanged | All configured gates pass; a receipt fingerprints executable, rule, dependency, and configuration files, including toolkit runtime/config. Prose-only docs and skill edits keep it valid. Durations go to a ledger; runs slower than usual are flagged. |
